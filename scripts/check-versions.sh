...
PROVIDER_TESTS_PLUGIN_VERSION=$(grep "sbt-scalapact" example/provider_tests/project/plugins.sbt | awk '{print $5}' | sed -e 's/[:space:,\")]//g')


ALL_VERSIONS="$CORE_VERSION $CONSUMER_VERSION $CONSUMER_PLUGIN_VERSION $PROVIDER_PLUGIN_VERSION $PROVIDER_TESTS_VERSION $PROVIDER_TESTS_PLUGIN_VERSION"
...
if [[ $ALL_VERSIONS == $ALL_EXPECTED ]]; then
  ...
  echo "> provider_tests plugin: $PROVIDER_TESTS_PLUGIN_VERSION"
