...
object ScalaPactHttpClient extends IScalaPactHttpClient[Task] {
  ...
  implicit val caller: Caller =
    Http4sClientHelper.doRequest(Http4sClientHelper.buildPooledBlazeHttpClient(maxTotalConnections, 2.seconds))

  def doRequest(simpleRequest: SimpleRequest)(implicit sslContextMap: SslContextMap,
                                              caller: Caller): Task[SimpleResponse] =
    caller(simpleRequest)

  def doInteractionRequest(url: String,
                           ir: InteractionRequest,
                           clientTimeout: Duration,
                           sslContextName: Option[String])(implicit sslContextMap: SslContextMap,
                                                           caller: Caller): Task[InteractionResponse] =
    caller(
      SimpleRequest(
        url,
  ...
  def doRequestSync(simpleRequest: SimpleRequest)(implicit sslContextMap: SslContextMap,
                                                  caller: Caller): Either[Throwable, SimpleResponse] =
    doRequest(simpleRequest).attemptRun.toEither

  def doInteractionRequestSync(url: String,
                               ir: InteractionRequest,
                               clientTimeout: Duration,
                               sslContextName: Option[String])(implicit sslContextMap: SslContextMap,
                                                               caller: Caller): Either[Throwable, InteractionResponse] =
