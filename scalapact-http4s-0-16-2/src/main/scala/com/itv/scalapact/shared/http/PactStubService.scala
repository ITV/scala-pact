...
object PactStubService {
  ...
  implicit class BlazeBuilderPimper(blazeBuilder: BlazeBuilder)(implicit sslContextMap: SslContextMap) {
    def withOptionalSsl(optContextNameAndClientAuth: Option[ContextNameAndClientAuth]): BlazeBuilder = optContextNameAndClientAuth.fold(blazeBuilder)(ssl => blazeBuilder.withSSLContext(sslContextMap.getContext(ssl.name), ssl.clientAuth))
  }
