  ...
  def verify(pactVerifySettings: PactVerifySettings, scalaPactSettings: ScalaPactSettings): Boolean = {
    ...
    pactVerifySettings match {
      ...
      case settings: PrePactsForVerificationSettings =>
        new PrePactsForVerificationVerifier(pactBrokerClient, localVerificationClient).verify(settings, scalaPactSettings)
...
object Verifier {
  def apply(implicit pactReader: IPactReader,
                  pactWriter: IPactWriter,
                  httpClient: IScalaPactHttpClientBuilder): Verifier =
