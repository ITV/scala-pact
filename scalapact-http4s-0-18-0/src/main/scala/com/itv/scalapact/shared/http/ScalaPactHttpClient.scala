...
object ScalaPactHttpClient extends IScalaPactHttpClient {
  ...
  def doRequestIO(performRequest: (SimpleRequest, Client[IO]) => IO[SimpleResponse], simpleRequest: SimpleRequest)(implicit sslContextMap: SslContextMap): IO[SimpleResponse] =
  ...
  def doInteractionRequestIO(performRequest: (SimpleRequest, Client[IO]) => IO[SimpleResponse], url: String, ir: InteractionRequest, clientTimeout: Duration, sslContextName: Option[String]): IO[InteractionResponse] =
